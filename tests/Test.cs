...
namespace tests
{
    ...
    public class Test
    {
        ...
        [Test()]
        public void TestWhenASDUCreatedFromByteArrayAsByteArrayReturnsCorrectValues()
        {
            var parameters = new ApplicationLayerParameters();
            var msgLength = 16;
            var bufPos = 6;
            // first 6 byte is the header
            var msg = new byte[] { 104, 14, 0, 0, 0, 0, 100, 1, 6, 0, 1, 0, 0, 0, 0, 20 };

            var expected = new byte[] { 100, 1, 6, 0, 1, 0, 0, 0, 0, 20 };

            var asdu = new ASDU(parameters, msg, bufPos, msgLength);

            Assert.AreEqual(expected, asdu.AsByteArray());
